...
sub summary : Private {
    ...
    my %dates = (
      year_ago => [ 'years', 1 ],
      three_months_ago => [ 'months', 3 ],
      month_ago => [ 'months', 1 ],
      week_ago => [ 'weeks', 1 ],
    );

    my $dtf = $c->model('DB')->storage->datetime_parser;
    for my $label ( keys %dates ) {
        my $date = DateTime->now->subtract(@{$dates{$label}});
        $c->stash->{$label} = $dtf->format_date($date);
    }

    # required to stop errors in generate_grouped_data
    $c->stash->{q_state} = '';
    $c->stash->{ward} = $c->get_param('q_ward'); 
